...
void animation_step(uint8_t payload [])
{
	...
	{	
		...
		for(uint8_t i=0; i<=lamp_size; i++)
		{
			...
			{
				if(mode_11.y_val_soll[i]*5 >= 255)
				{
					mode_11.y_val_ist[i] = 255;
					mode_11.y_col_ist[i] = 255;
				}
				else
				{
					mode_11.y_val_ist[i] = mode_11.y_val_soll[i]*5;
					mode_11.y_col_ist[i] = mode_11.y_val_soll[i]*5;
				}
			}
			...
			else if((mode_11.y_val_ist[i]>0) && (mode_11.y_val_ist[i]<=2))
			{
		...
		for(uint8_t i=0; i<lamp_size; i++)
		{
			for(uint8_t j=dots; j<dots+6; j++)
			{
				if(i<2)
				{
					if((40+mode_11.y_col_ist[i])>=255)
					{
						setLED_HSV(j, color-addC, 255, mode_11.y_val_ist[i]);	
					}
					else
					{
						setLED_HSV(j, color-addC, 40+mode_11.y_col_ist[i], mode_11.y_val_ist[i]);
					}
				}
				else
				{
