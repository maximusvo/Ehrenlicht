...
void anim_frame()
{
	...
	static uint8_t change = 1, color, count = 1, tempH = 200, brightDimm = 255, length = 3;;
	static uint16_t bandD[ANIM_BAND_NUM];
	//static int8_t brightDimmAdd = -15; //TESTEN****************
	static float colorSec = 1.0f;
	float ampli = 0;
	uint8_t y = 0, stop, brightNext, ampliFinal, plus = 0, freqHigh = 0;
	uint16_t band[length], bandDD[ANIM_BAND_NUM];
	...
	else
	{
		...
		bandDD[0] = bandD[0]/count;
		bandDD[1] = bandD[1]/count;
		bandDD[2] = bandD[2]/count;
		bandDD[3] = bandD[3]/count;
		bandDD[4] = bandD[4]/count;
		bandDD[5] = bandD[5]/count;
		bandDD[6] = bandD[6]/count;
	}
	band[2] = bands_l[1];
		
	// umrechnung der frequenzdaten
	band[1] /= 4;
	stop = maxVal(band, length);
	band[2] = (band[2]/band[stop]) * 100;
	band[1] = (band[1]/band[stop]) * 100;
	band[0] = (bands_l[6]/bandDD[6]) * 100;
	// 12 bit pegel in logarithmische skala umrechnen
	if(amplitude_l > 1)
	{
		ampli = log10 (amplitude_l); 
		ampli *= 3; // Skalierungsfaktor
	}
	else
	{
		ampli = 0; 
	...
	stop = 0;
	//if(band[1] >= 96) //anpassen!! 100
	//{
		//stop = 1;
	//}
	freqHigh = 2;
	if((band[2] >= 1)) //& !stop anpassen!! 103
	{
		freqHigh = 3;	
	}
...
//////*/
	freqHigh = maxVal(bandDD, ANIM_BAND_NUM);
	// finde aktuelle bpm 
	if((bpm_l >= 35) && (bpm_l <= 110))//((bpm_h >= 35) && (bpm_h <= 110)) |  )
	{
		colorSec = 1.0f; //naechster farbwechsel in xy * ms
		
	}
	if((bpm_l >= 111) && (bpm_l <= 150))//((bpm_h >= 111) && (bpm_h <= 150)) |  )
	{
		colorSec = 0.6f;
	}
	if((bpm_l >= 151) && (bpm_l <= 180))//((bpm_h >= 151) && (bpm_h <= 180)) |  )
	{
		colorSec = 0.2f;
	}
	else
	...
	if(change)
	{
		if(beats) //&& (brightDimm == 255))
		{
			...
			switch(freqHigh)
			{
				case 0: color = 250; break;
				case 1: color = 200; break;
				case 2: color = 150; break; //fuer umrechnung in hue-winkel wert*1,434, umgekehrt winkel auf 1,434 teilen
				case 3: color = 100; break;
				case 4: color = 50; break;
				case 5: color = 30; break;
